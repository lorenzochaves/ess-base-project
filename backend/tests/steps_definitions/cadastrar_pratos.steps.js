...
Given('que eu sou um administrador do sistema', function () {
  // Nada precisa ser feito aqui, pois é um passo de contexto.
});

// Passo: Fazer uma requisição POST para "/api/dishes" com os dados
When('uma requisição POST é feita para {string} com os dados:', async function (endpoint, dataTable) {
...
When('uma requisição GET é feita para {string}', async function (endpoint) {
  try {
...
Then('o código da resposta deverá ser {int}', function (statusCode) {
...
Then('a mensagem de erro deve ser {string}', function (errorMessage) {
  assert.ok(response, 'A resposta não foi definida.');
...
Then('a mensagem de erro deve ser {string} e {string}', function (error1, error2) {
  assert.ok(response, 'A resposta não foi definida.');
